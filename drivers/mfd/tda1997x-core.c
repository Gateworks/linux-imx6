...
*/
	...
	regulator_put(tda1997x->dvdd_reg);
	regulator_put(tda1997x->avdd_reg);
	regulator_put(tda1997x->dvddio_reg);
			 ...
			 const struct i2c_device_id *id)
{
	...
	u8 reg;
	struct tda1997x_data *tda1997x = &tda1997x_data;
	struct tda1997x_platform_data *pdata = client->dev.platform_data;
  ...
  mutex_init(&tda1997x->cec_lock);
	...
	if (pdata->dvddio_reg) {
		tda1997x->dvddio_reg = regulator_get(&client->dev, pdata->dvddio_reg);
		if (!IS_ERR(tda1997x->dvddio_reg)) {
			regulator_set_voltage(tda1997x->dvddio_reg, 3300000, 3300000);
			if (regulator_enable(tda1997x->dvddio_reg) != 0)
				return -ENODEV;
		}
	}

	if (pdata->dvdd_reg) {
		tda1997x->dvdd_reg = regulator_get(&client->dev, pdata->dvdd_reg);
		if (!IS_ERR(tda1997x->dvdd_reg)) {
			regulator_set_voltage(tda1997x->dvdd_reg, 1800000, 1800000);
			if (regulator_enable(tda1997x->dvdd_reg) != 0)
				return -ENODEV;
		}
	}

	if (pdata->avdd_reg) {
		tda1997x->avdd_reg = regulator_get(&client->dev, pdata->avdd_reg);
		if (!IS_ERR(tda1997x->avdd_reg)) {
			regulator_set_voltage(tda1997x->avdd_reg, 1800000, 1800000);
			if (regulator_enable(tda1997x->avdd_reg) != 0)
				return -ENODEV;
		}
	}
	...
	if (client->irq) {
		...
		pr_debug("registered irq for gpio%d\n", client->irq);
	}
...
MODULE_AUTHOR("Tim Harvey <tharve@gateworks.com>");
MODULE_LICENSE("GPL");
