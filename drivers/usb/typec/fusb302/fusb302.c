...
/* backport wrappers */
#define device_property_read_u32(dev, prop, v) \
	of_property_read_u32(dev->of_node, prop, v)
#define device_property_read_string(dev, prop, v) \
	of_property_read_string(dev->of_node, prop, v)
#define extcon_get_state(x, y) extcon_get_cable_state_(x, y)
// TODO: check this see 11eecf910
...
#define seq_has_overflowed(s)	(s->count == s->size)
		...
		const struct power_supply_config *cfg)
{
	...
	desc->drv_data = desc;
	ret = power_supply_register(parent, desc);
	if (ret < 0) {
		devres_free(ptr);
		return ERR_PTR(ret);
	} else {
...
// FIXME: sync these up with 3.14's extcon definitions
//  - I think this would only get triggered via drivers/usb/phy.c which we
//    dont have on this port so this whole function is likely a dont-care
	do {
...
static struct power_supply fusb302_psy_desc = {
