/*
 * drivers/media/video/mxc/capture/tda1997x.c -- MXC video capture driver 
 *          for i.MX boards with tda1997x HDMI receiver
 *
...
static int __devinit tda1997x_video_probe(struct platform_device *pdev)
{
	struct sensor *sens = &tda1997x_data;
	int ret = 0;
	struct fsl_mxc_tvin_platform_data *pdata = pdev->dev.platform_data;
...
static int __devexit tda1997x_video_remove(struct platform_device *pdev)
{
...
static int __devinit tda1997x_video_init(void)
{
	return platform_driver_register(&tda1997x_video_driver);
}
module_init(tda1997x_video_init);

static void __devexit tda1997x_video_exit(void)
{
	platform_driver_unregister(&tda1997x_video_driver);
}
module_exit(tda1997x_video_exit);
